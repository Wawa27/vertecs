...
export default class Entity {
  ...
  public constructor(options?: EntityOptions) {
    ...
    if (SystemManager.getInstance().entities.find(entity => entity.id === id)) {
  ...
  public static findById(id: string): Entity | undefined {
    return SystemManager.getInstance().entities.find(entity => entity.id === id);
  }
   ...
   */
  public static findByComponent(component: ComponentClass): Entity | undefined {
    return SystemManager.getInstance().entities.find(entity => entity.getComponent(component));
  }
  ...
  public static findAllByComponent(component: ComponentClass): Entity[] {
    return SystemManager.getInstance().entities.filter(entity => entity.getComponent(component));
  }
  ...
  public static findAllByTag(tag: string): Entity[] {
    return SystemManager.getInstance().entities.filter(entity => entity.tags.includes(tag));
  }
   ...
   */
  public findComponent<T extends Component>(componentConstructor: ComponentClass<T>): T | undefined {
   ...
   * @param filter
   */
  public getComponents<T extends Component>(filter?: ComponentClass<T>[]): (T | undefined)[] {
    if (!filter) {
  ...
  public addComponent(component: Component) {
    if (!this.getComponent(component.constructor as ComponentClass)) {
      SystemManager.getInstance().onComponentAddedToEntity(this, component);
   ...
   */
  public removeComponent<T extends Component>(componentClass: ComponentClass<T>): T | undefined {
    const component = this.getComponent(componentClass);
    if (component) {
      this.#components.splice(this.#components.indexOf(component), 1);
      SystemManager.getInstance().onComponentRemovedFromEntity(this, component);
      return component;
    }
  ...
  public destroy(): void {
    for (let i = this.components.length - 1; i >= 0; i--) {
      this.removeComponent(this.components[i].constructor as ComponentClass);
  ...
  public set parent(entity: Entity | undefined) {
    ...
    this.components.forEach(component => component.onEntityParentChanged(this));
  }
