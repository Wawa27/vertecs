...
class DefaultNarrowphase {
    ...
    getSphereSphereCollision(sphere1Position, sphere1Shape, sphere2Position, sphere2Shape) {
        ...
        const contactNormal = new Vec3$1();
...
class BodyDebugger extends Component {
    ...
    onAddedToEntity(entity) {
        ...
        vec3.subtract(vec3.create(), boundingBox?.maximum || [0, 0, 0], boundingBox?.minimum || [0, 0, 0]);
        boundingBoxDebugEntity.addComponent(new ThreeObject3D(new Mesh(new SphereGeometry(sphereBody?.radius || 1, 32, 32), new MeshBasicMaterial({ color: 0xff0000 }))));
        const inverseScale = vec3.create();
        vec3.inverse(inverseScale, entity.getComponent(Transform)?.getWorldScale() || [1, 1, 1]);
...
class Particle extends Component {
    ...
    clone() {
        return new Particle({
            ...
            timeAlive: 0,
            startColor: new Vec3$1(this.startColor),
            endColor: new Vec3$1(this.endColor),
        });
    }
}
    ...
    onLoop(components, entities, deltaTime) {
        for (let i = components.length - 1; i >= 0; i--) {
            ...
            if (particle.timeAlive >= particle.lifeTime) {
                ...
                transform.setWorldScale(new Vec3$1(particle.startScale +
                    (particle.endScale - particle.startScale) *
                        (particle.timeAlive / particle.lifeTime)));
                const worldPosition = new Vec3$1(transform.getWorldPosition());
                worldPosition.add([
...
export { AnimatedState, AnimatedStateSystem, Animation, Body, BodyDebugger, Broadphase, BruteForceBroadphase, ClientComponent, ClientHandler, ClientNetworkSystem, Collision, Component, DefaultNarrowphase, DefaultSolver, EcsManager, Entity, FiniteStateMachine, FiniteStateMachineSystem, IoUtils, IsNetworked, IsPrefab, KeyboardInputSystem, MathUtils, MouseInputSystem, NetworkAnimation, NetworkComponent, NetworkSphereBody as NetworkCubeBody, NetworkFiniteStateMachine, NetworkSphereBody$1 as NetworkSphereBody, NetworkTransform, OimoComponent, OimoSystem, Particle, ParticleEmitter, ParticleSystem, PhysicsSystem, PrefabManager, Ray, SerializableComponent, SerializedEntity, ServerNetworkSystem, Sound, SoundSystem, SphereBody, State, StateSystem, System, ThreeAnimation, ThreeAnimationSystem, ThreeCamera, ThreeCameraSystem, ThreeComponentDebugger, ThreeCss3dComponent, ThreeCss3dSystem, ThreeInstancedMesh, ThreeLightComponent, ThreeLightSystem, ThreeObject3D, ThreeSystem, TimedState, TimedStateSystem, Transform };
