...
describe("Networking", async () => {
    ...
    let serverNetworkSystem;
    const serverEcsManager = new EcsManager();
    let clientANetworkSystem;
    const clientAEcsManager = new EcsManager();
    let clientBNetworkSystem;
    const clientBEcsManager = new EcsManager();
    it("should start server", async () => {
        serverNetworkSystem = new ServerNetworkSystem(allowedNetworkComponents, TestClientHandler);
    ...
    it("should connect to server", async () => {
        ...
        clientANetworkSystem = new TestClientNetworkSystem(allowedNetworkComponents, "ws://localhost:8080");
    ...
    it("should send the newly created entity to client", async () => {
        ...
        assert.equal(newClientEntity.getComponent(IsNetworked)?.ownerId, clientANetworkSystem.networkId);
    });
    it("should update and send the new count", async () => {
        const clientACounterEntity = clientANetworkSystem.entities[0];
        const serverCounterComponent = serverEcsManager.entities[0].getComponent(CounterComponent);
        serverCounterComponent?.increment();
        await new Promise((resolve) => setTimeout(resolve, 100));
        assert.equal(clientANetworkSystem.entities.length, 1);
        assert.exists(clientACounterEntity);
        assert.equal(clientACounterEntity.getComponent(CounterComponent)?.count, 1);
    });
    it("should send all up to date entities to new clients", async () => {
        clientBNetworkSystem = new TestClientNetworkSystem(allowedNetworkComponents, "ws://localhost:8080");
    ...
    it("should allow clientA to update the count", async () => {
        ...
        const clientBCounterComponent = clientBNetworkSystem.entities[0].getComponent(CounterComponent);
        const serverCounterComponent = serverEcsManager.entities[0].getComponent(CounterComponent);
    ...
    it("should send private data to clientB", async () => {
        ...
        serverNetworkSystem.sendCustomDataToClient(clientBNetworkSystem.networkId ?? "", message);
        await new Promise((resolve) => setTimeout(resolve, 100));
