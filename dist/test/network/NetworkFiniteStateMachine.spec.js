...
import { EcsManager, Entity, FiniteStateMachine, FiniteStateMachineSystem, IsNetworked, NetworkFiniteStateMachine, PrefabManager, ServerNetworkSystem, State, TimedState, TimedStateSystem, } from "../../src";
...
describe("FiniteStateMachine", async () => {
    ...
    before(async () => {
        ...
        const finiteStateMachine = new FiniteStateMachine("test1", [
    ...
    it("should start with state1", async () => {
        ...
        const finiteStateMachine = clientAEntity.getComponent(FiniteStateMachine);
        assert.equal(finiteStateMachine?.currentStateName, "test1");
    });
    it("should move to state2 after a second", async () => {
        await new Promise((resolve) => setTimeout(resolve, 1000));
        const clientAEntity = clientAEcsManager.entities[0];
        const clientAFiniteStateMachine = clientAEntity.getComponent(FiniteStateMachine);
