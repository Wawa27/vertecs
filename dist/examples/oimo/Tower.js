import { BoxGeometry, Mesh, MeshStandardMaterial, PerspectiveCamera, PointLight, SphereGeometry, } from "three";
import { quat } from "ts-gl-matrix";
import { OimoComponent, OimoSystem, ThreeCamera, ThreeLightComponent, ThreeObject3D, Transform, } from "../../src";
...
floor.addComponent(new ThreeObject3D(new Mesh(new BoxGeometry(50, 10, 50), new MeshStandardMaterial({ color: 0x808080 }))));
...
light.addComponent(new ThreeLightComponent(new PointLight(0xffffff, 1, 200, 0)));
const addTower = (towerOptions) => {
    if (towerOptions.radius > 45)
        return;
    ...
    for (let j = 0; j < height; j++) {
        for (let i = 0; i < detail; i++) {
            ...
            brick.addComponent(new Transform([px, py, pz], quat.fromEuler(quat.create(), 0, angle * (180 / Math.PI), 0)));
            brick.addComponent(new ThreeObject3D(new Mesh(new BoxGeometry(sx, sy, sz), new MeshStandardMaterial({ color: 0x808080 }))));
            brick.addComponent(new OimoComponent({
                type: "box",
                move: true,
                size: [sx, sy, sz],
                density: mass,
...
platform.addComponent(new Transform([-30, 10, 0], quat.fromEuler(quat.create(), 0, 0, 60)));
platform.addComponent(new ThreeObject3D(new Mesh(new BoxGeometry(2, 40, 50), new MeshStandardMaterial({ color: 0x808080 }))));
platform.addComponent(new OimoComponent({
    size: [2, 40, 50],
    density: 1,
    restitution: 0.2,
}));
...
ball.addComponent(new ThreeObject3D(new Mesh(new SphereGeometry(2, 64, 64), new MeshStandardMaterial({ color: 0x808080 }))));
ball.addComponent(new OimoComponent({
    type: "sphere",
    move: true,
    size: [2, 2, 2],
    density: 0.6,
